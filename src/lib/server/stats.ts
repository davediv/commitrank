...
import { calculateNextHourlySync } from './sync-config';

/** One complete cache contract for the homepage and public stats API. */
export async function queryStats(db: Database): Promise<StatsResponse> {
	const now = new Date();
